...
use expressions::{ExprValue, ExprBinaryOp};
              ...
              let (_, insts) = typed.view().apply(&|op| {
                match op {
                  Op::Value(val) => {
                    ...
                    match val {
                      ExprValue::Literal(i) => (to_use, vec![format!("load (v{}, int), ({}, int)", to_use, i)]),
                      ExprValue::Variable(v) => (to_use, vec![format!("load (v{}, int), ({}, {})", to_use, v.name, v.t)])
                    }
                  },
                  Op::BinaryOp(op, (fst_var, mut fst_instructs), (snd_var, mut snd_instructs)) => {
                    let op_name = match op {
                      ExprBinaryOp::Add => "add",
                      ExprBinaryOp::Subtract => "sub",
                      ExprBinaryOp::Multiply => "mul",
                      ExprBinaryOp::Divide => "div",
                      ExprBinaryOp::BitShiftLeft => "shl",
                      ExprBinaryOp::BitShiftRight => "shr",
                      ExprBinaryOp::BitwiseAnd => "and",
                      ExprBinaryOp::BitwiseXor => "xor",
                      ExprBinaryOp::BitwiseOr => "or"
                    };
