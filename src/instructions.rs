...
impl<T: std::fmt::Debug + std::clone::Clone> std::fmt::Debug for Expr<T> {
  ...
  pub fn new(tokens_stack: &mut Vec<Tokens>) -> Option<Instruction> {
    ...
    match &current {
        ...
        match tokens_stack.pop().unwrap() {
          Tokens::AssignSymbol => {
            if let Some(mut snd) = Instruction::new(tokens_stack) {
              ...
              let aux = Expr::new(tokens_stack).unwrap();
          ...
          val => {
            ...
            let instruction = Instruction {
              nodes: vec![
                Op::Value(InstructionValue::Expression(Expr::new(tokens_stack).unwrap())),
              ],
              head: 0
            };
            match tokens_stack.pop()? {
      ...
      Tokens::Return => {
        let instruction = Instruction {
          nodes: vec![
            Op::Value(InstructionValue::Expression(Expr::new(tokens_stack)?)),
